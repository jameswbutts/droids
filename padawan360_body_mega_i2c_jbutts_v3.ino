...
Sparkfun MP3 Trigger (jbutts: REPLACED WITH ADAFRUIT SOUNDFX BOARD)
...
const byte DOMEDEADZONERANGE = 22;
...
int automateDelayMin = 6;
int automateDelayMax = 36;
int automateDelay = automateDelayMin;
//How much the dome may turn during automation.
int turnDirectionAutoMin = 8;
int turnDirectionAutoMax = 30;
int turnDirection = turnDirectionAutoMin;
...
#define CHATPAD //uncomment if using chatpad

void setup() {
  delay(100);
  Serial3.begin(SABERTOOTHBAUDRATE);
  Serial2.begin(DOMEBAUDRATE);
  Serial1.begin(SFXBAUDRATE);
  Serial.begin(115200);
...
void loop() {
  ...
  if (isInAutomationMode) {
    ...
    if (currentMillis - automateMillis > (automateDelay * 1000)) {
      ...
      if (automateAction < 4) {
        ...
        if (turnDirection > 0) {
          turnDirection = random(turnDirectionAutoMin, turnDirectionAutoMax) * -1;
        } else {
          turnDirection = random(turnDirectionAutoMin, turnDirectionAutoMax);
        }
      }

      // sets the mix, max seconds between automation actions - sounds and dome movement
      //automateDelay = random(3,10);
      automateDelay = random(automateDelayMin, automateDelayMax);
    }
  }

  // Volume Control of MP3 Trigger
  // Hold R1 and Press Up/down on D-pad to increase/decrease volume
  // Not sure why R1 not working - jbutts
  ...
  if (Xbox.getButtonClick(X, 0)) {
    ...
    if (Xbox.getButtonPress(L1, 0)) {
      ...
      //sfx.playTrack(4); //remove wolf whistle
      //logic lights
      triggerI2C(10, 4);
    } else if (Xbox.getButtonPress(R1, 0)) {
      sfx.playTrack(12);
  ...
  if (isDriveEnabled) {
    ...
    // jbutts WIP
    // reverse backup left/right
    if (driveThrottle < 0) {
      if (turnThrottle > 0) {
        //turnThrottle = turnThrottle * -1;
      }
    }
...
void triggerI2Castro(const char* command) {
  Wire.beginTransmission(i2cAstroDeviceID);
  Wire.write(command);
  Wire.endTransmission();
}
...
void Check_Chatpad() {
    ...
    if (Xbox.getChatpadClick(XBOX_CHATPAD_D1, 0)) {
       //Duel of the Fates 
       sfx.playTrack(12); // Duel of Fates 4m 17s
    }   

    if (Xbox.getChatpadClick(XBOX_CHATPAD_D2, 0)) {
         //Theme
        sfx.playTrack(9); // Star Wars Theme 5m 29s
    }   

    if (Xbox.getChatpadClick(XBOX_CHATPAD_D3, 0)) {
       //Imperial March
       sfx.playTrack(11); // Imperial March 3m 5s
    }   
    
    if (Xbox.getChatpadClick(XBOX_CHATPAD_D4, 0)) {
       //Cantina
       sfx.playTrack(10); // Cantina 2m 50s
    }   
    ...
    if (Xbox.getChatpadClick(XBOX_CHATPAD_D7, 0)) {
       //Muppets
       sfx.playTrack(58);// Muppets Tune 30s
    }


    if (Xbox.getChatpadClick(XBOX_CHATPAD_D8, 0)) {
       //Leia Message Snippet
       sfx.playTrack(56);
    }   
